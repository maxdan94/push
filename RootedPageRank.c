...
#define NITER 100 //Number of power iterations to compute pagerank
...
double* pagerank(sparse* g, unsigned long source, double alpha, unsigned char k){
	double *v1,*v2,*v3;
	double s;
	unsigned char i;
...
void printres(FILE* file,unsigned long n, double* vect, double eps){
	unsigned long i,n0=0,n_eps=0;
	double sum=0,sum2=0;
	for (i=0;i<n;i++){
		sum+=vect[i];
		if (vect[i]>0){
	...
	printf("Number of entries greater than epsilon: %lu\n",n_eps);
	printf("Sum of values greater then epsilon: %le\n",sum2);
}
...
int main(int argc,char** argv){
	...
	pr=pagerank(g,source,ALPHA,NITER);
